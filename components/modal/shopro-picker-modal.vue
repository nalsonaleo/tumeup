		...
		<view class="cu-dialog shop-modal" @tap.stop style="background: none;">
			<view class="modal-box page_box">
				<view class="modal-head x-bc">
					<text></text>
					<text class="head-title">{{$t('components.shoproPickerModal.xzyhq')}}</text>
				...
				<view class="modal-content content_box y-f">
					<label class="radio-item x-bc" @tap="selCoupon(0)">
						<text class="coupon-title">{{$t('components.shoproPickerModal.bqsyhq')}}</text>
					...
					<label class="radio-item x-bc" v-for="(radio, index) in pickerData.couponList" :key="radio.id" @tap="selCoupon(index + 1,radio.id)">
						...
						<text class="coupon-title"  v-if="radio.c_type == 2">{{ radio.c_name }}:{{ `${$t('components.shoproPickerModal.man')}${radio.c_full}${$t('components.shoproPickerModal.jian')}${radio.c_reduce}` }}</text>
						<text class="coupon-title"  v-if="radio.c_type == 3">{{ radio.c_name }}:{{  parseFloat(radio.c_discount)  }}{{$t('components.shoproPickerModal.zhe')}}</text>
				...
				<view class="modal-foot x-c"><button class="cu-btn serve-btn" @tap="saveCoupon">{{$t('components.shoproPickerModal.qd')}}</button></view>
			</view>
		</view>
	</view>
</template>
		...
		selCoupon(index,id) {
			...
			if(uni.getStorageSync('couponId')){
				JSON.parse(uni.getStorageSync('couponId')).forEach(function(item){
					...
					if(item == id){
						that.$msg(that.$t('components.shoproPickerModal.gyhqybxz'));
						return false;
					}else{
