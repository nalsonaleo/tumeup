		...
		<view class="detail_box">
			<view class="detail-content">
				<view class="goodes_detail_swiper-box">
					...
					<swiper class="carousel" circular @change="swiperChange" :autoplay="true">
						<swiper-item  v-for="(img, index) in goodsInfo.g_imgs" :key="index" class="carousel-item">
							<image class="swiper-image" :src="img" mode="aspectFill" lazy-load></image>
						</swiper-item>
					</swiper>
					<view v-if="goodsInfo.g_imgs" class="swiper-dots">{{ swiperCurrent + 1 }} / {{ goodsInfo.g_imgs.length }}</view>
				</view>

				<!-- 价格卡片组 -->
				<sh-price v-if="goodsInfo" :detail="goodsInfo"  @change="getActivityRules"></sh-price>
				<view style="display: flex;justify-content: center;align-items: center;width: 100%;background-color: #fff;">
					<view>
				...
				<!-- 优惠券 -->
				<view class="coupon-list" v-if="couponsCan.length && goodsInfo.g_type == 2" v-for="c in couponsCan" :key="c.id">
			...
			<view class="detail-foot_box  x-f" v-if="goodsInfo.g_type != 3 && !showSku">
				<view class="left x-f">
					...
					<view v-if="goodsInfo.g_type != 4 && goodsInfo.g_type != 1" class="tools-item y-f" @tap="onFavorite(goodsInfo.id)">
						<image
							class="tool-img"
