				...
				<view v-for="(item, index) in dataList" :key="index" class="floor-item" @click="navToGoodsPage(item)">
					...
					<view class="content">
						...
						<view class="weui-flex">
							...
							<view class="buttons">
								<text class="sku_btn" v-if="item.yuding || !item.isCanAddToCart">{{$t("index.miaosha.see")}}</text>
								<text class="sku_btn" v-else-if="item.hasSku" @click.stop="openSku(item)">{{$t("index.normal.select")}}</text>
