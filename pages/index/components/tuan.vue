			...
			<view class="seckill-section2">
				<view v-for="(item,index) in dataList" :key="index" @click="navToGoodsPage(item)" class="sec2 sec3">
					<view class="image-content">
						<image :src="item.banner | thumbImg('710x350')" mode="aspectFill" lazy-load="true" class="img"></image>
					</view>
					<view class="content">
						...
						<view class="price-area">
							...
							<text class="sku_btn yticon icon-gouwuche_" v-if="item.isCanAddToCart" @click.stop="saveCartAmount(item, 0)"></text>
							<text class="sku_btn" v-else>{{$t("index.miaosha.see")}}</text>
						</view>
						<view class="miaoshaTime basc">
							<text>{{$t("index.tuan.extraction")}}：</text><text>{{item.tuangou.pickup | dateFormat('MM-dd hh:mm')}}</text>
						</view>
					</view>
				</view>
			</view>
