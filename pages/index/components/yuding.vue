					...
					<view class="price-area">
						<view class="pa">
							<view class="miaoshaTime" v-if="item.yuding.isBegin">
								<text>{{$t("index.miaosha.endtime")}}：</text><text>{{item.yuding.endTime | dateFormat('MM-dd hh:mm')}}</text>
							</view>
							<view class="miaoshaTime basc" v-if="!item.yuding.isBegin">
								<text>{{$t("index.miaosha.starting")}}：</text><text>{{item.yuding.beginTime | dateFormat('MM-dd hh:mm')}}</text>
							</view>
							<text class="price">{{ item.price }}</text>
							<text class="price del" v-if="item.originPrice > 0">{{ item.originPrice }}</text>
						</view>
						<text class="sku_btn">{{$t("index.miaosha.see")}}</text>
					</view>
				</view>
			</view>
		</view>
