	...
	export default {
		...
		getUrlParam(name) {
		   ...
		   computed: {
		   	...
		   	enableVipCard() {
		   		if (!this.$config.vipCard) {
		   			return false;
		   		}
		   		return this.$config.vipCard.enable;
		   	},
		   	editVipCard() {
		   		if (!this.$config.vipCard) {
		   			return false;
		   		}
		   		return this.$config.vipCard.edit;
		   	},
		   	vipCardBg() {
		   		if (!this.$config.vipCard) {
		   			return "";
		   		}
		   		return this.$config.vipCard.bgImage;
		   	},
		   	applyTuanZhang() {
		   		if (!this.$config.selfRaising) {
		   			return false;
		   		}
		   		return this.$config.selfRaising.apply;
		   	},
		   	locales() {
		...
		methods:{
			...
			async loadData() {
				...
				if (this.hasLogin) {
					...
					getUserStatistics({}).then(res => {
						...
						this.selfraising = res.selfraising;
						if (res.order) {
					...
					if (this.userInfo.invite > 0) {
						this.teamTips = this.$t('user.jointeam') + ":" + this.userInfo.invite;
					}
					getGoodsVisites({
						shopid: this.shopId
					}).then(
						res => {
							this.hasVisiteGoods = res && res.length > 0;
					...
					this.teamTips = this.$t('user.jointeam');
				}
			},
			previewAvatar() {
				if (!this.hasLogin) {
			...
			openVip() {
				...
				//已经存在了会员卡信息，如果禁用修改，则不允许再进入
				if (!this.editVipCard && this.userInfo.id) {
					return;
				}
				this.navToItem("/pages/user/vip/add", this.userInfo.member);
				/* saveUserInfo({
					channel: "free"
				}, "vip").then(res => {
					this.loadData();
			...
			checkVip() {
				...
				if (n > 1) {
					...
					saveUserInfo({
						...
						this.loadData();
						this.$api.msg(this.$t('user.opened'), 1500, true, "success");
					}, err => {
						this.$api.msg(err.message)
					});
				} else {
					this.$api.msg(this.$t('user.input'))
				}
			},
			//加入团队/设置邀请者
			joinTeam() {
			...
			addInvite() {
				...
				if (n > 1) {
					...
					saveUserInfo({
						...
						this.loadData();
						this.$api.msg(this.$t('user.join'), 1500, true, "success");
					}, err => {
						this.$api.msg(err.message)
					});
				} else {
			...
			navToShopOrder(item, state) {
				...
				uni.navigateTo({
					url: `/pages/admin/admin?shopid=${id}&second=Order&state=${state}`
				})
			},
			navToShopHome(item) {
				item.shopFixedId = item.id;
				this.changeShopId(item);
				uni.switchTab({
					url: `/pages/index/fruit`
				})
			},
			navToSelfRaisingAdmin(item) {
				this.navToItem('/pages/selfraisingpoint/admin', item);
			},
			getUserInfo() {
