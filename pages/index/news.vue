		...
		<view class="problem-item" v-for="(item, index) in faqList" :key="item.id" @tap="onProblem(index)">
			<view class="x-bc title-box">
				<view class="x-f">
					...
					<view class="title" style="display: flex;align-items: center;justify-content: space-between;font-size: 26rpx;">
						<text style="color: #000;font-size: 30rpx;">{{item.m_title}}</text>
						<text style="color: #999;font-size: 0.7rem;margin-left: 20upx">{{item.m_time}}</text>
					</view>
				</view>
