	...
	<view class="page">
		...
		<view class="express">
			<view class="top ali-c">
				<view class="text">
					<view class="text1">{{detail.nu}}</view> 
					<view class="text2">{{detail.express_name}}</view>
				</view>
			</view>
		</view>
		...
		<view class="content bgf">
			<view>
				<view class="flex list" :class="{one: index == 0 }" v-for="(item, index) in detail.data"
				 :key="index">
					<view class="time">
...
export default {
	...
	methods: {
		getOrderItemDetail() {
			...
			that.$api.logistics(data).then(res => {
				if(res.code == 1){
